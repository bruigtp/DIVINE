...
The `impute_missing()` function lets you replace or remove missing values in a single operation. You provide a named list of formulas (`<selector> ~ <value>`), where `<selector>` can be any tidyselect expression (e.g. a column name, `starts_with()`, or `where(is.numeric)`) and `<value>` one of the following strategies:
...
After running `impute_missing()`, the returned cleaned dataset has no missing values in the defined columns, according to the chosen strategy. The dataset will maintain the same structure, unless the argument `drop_all_na` is set to `TRUE`.

<br>

## 3. Merging Multiple Tables with `multi_join()`

When working with related tables, `multi_join()` merges several data frames by a common key. You can perform different join types (`"left"`, `"inner"`, `"right"`, `"full"`). For example, suppose you have demographic, vital signs, and scores tables all sharing the default key variables of the package (`record_id`, `covid_wave`, `center`):
