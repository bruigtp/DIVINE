...
Information on vital signs of patients included in the DIVINE cohort.
Data was collected at hospital admission.
...
- supporto2::
  ...
  A factor with levels `No` and `Yes`. Oxygen Support

- normal_radio::

  A factor with levels `No` and `Yes`. Normal X-ray

- pleural_effusion::

  A factor with levels `No` and `Yes`. Pleural effusion
